...
### 8. `dataframe_with_rigwipemessage.ipynb` + `Analyzing_dataframe.ipynb`
   ...
   - **Reccomendations**:
      - Prioritize Scrapping Unhealthy Drives (Failure Rate Above 50%): For failure messages with a failure rate above 50%, especially those indicating potential hardware issues, I recommend scrapping the drive. The likelihood of a successful rig wipe is low, and further attempts may waste valuable time.
      - Prioritize the troubleshooting and review of drives in this category to improve the chances of a successful wipe. This will ensure that operational resources are spent efficiently, focusing on drives that have the potential for successful wipes on the rig.
      - Create a Monitoring System for Failure Messages: Develop a system to automatically flag failure messages with high failure rates (above 50%) and provide alerts for further action. This can help operations teams make quicker decisions on whether to scrap drives or escalate them for further review.
